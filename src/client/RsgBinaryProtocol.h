...
/*
 * RSG NOTE: this is the file that you can find in /usr/include/thrift/protocol/TBinaryProtocol.h
 * We add somme virtual key words to be able to intercept some calls and do some printfs.
 */
...
public:
  ...
  /*ol*/virtual uint32_t writeMessageBegin(const std::string& name,
                                    const TMessageType messageType,
                                    const int32_t seqid);

  /*ol*/virtual uint32_t writeMessageEnd();
  ...
  /*ol*/ virtual uint32_t readMessageBegin(std::string& name, TMessageType& messageType, int32_t& seqid);
