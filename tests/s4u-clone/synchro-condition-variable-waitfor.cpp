...
void competitor(void * arg)
{
...

    {
        ...
        while (!ready) {
            RSG_INFO("About to call wait_for...");
            if (cv->wait_for(lock, 0.25*(*my_id+1)) == std::cv_status::timeout)
                RSG_INFO("Out of wait_for: timeout reached :(");
            else
                RSG_INFO("Out of wait_for: YAY notified!");
        }
